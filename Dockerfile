...
RUN apt install tar -y && tar -zxvf apache-tomcat-9.0.100.tar.gz
RUN mv apache-tomcat-9.0.100 tomcat
RUN mv /root/tomcat-Dockerfile/host/context.xml /usr/local/tomcat/webapps/host-manager/META-INF/context.xml
RUN mv /root/tomcat-Dockerfile/manager/context.xml /usr/local/tomcat/webapps/host-managermanager/META-INF/context.xml
